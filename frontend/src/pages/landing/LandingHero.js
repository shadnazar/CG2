/**
 * LandingHero.js
 * The hero/offer page for problem-specific landing pages
 * URL: /{slug}
 */
import React, { useState, useEffect, useRef } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { 
  Star, Check, Shield, Truck, Clock, Award,
  AlertCircle, Sparkles, ArrowRight, ChevronRight
} from 'lucide-react';
...
function LandingHero() {
  ...
  useEffect(() => {
    if (pageData) {
      ...
      trackViewContent(pageData.problem_title, PREPAID_PRICE);
      trackAction('landing_page_view', { slug, category: pageData.category });
  ...
  }, [pageData, slug, trackPageVisit, trackViewContent, trackAction]);
  ...
  const handleViewProduct = () => {
    trackAction('landing_cta_click', { slug, button: 'view_product' });
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <Link
            to="/"
            className="px-6 py-3 bg-purple-500 text-white rounded-xl font-semibold inline-block"
          >
            Go to Homepage
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen bg-white pb-24">
      ...
      <section className="bg-gradient-to-b from-purple-50 via-white to-white px-5 py-10">
        ...
        >
          View Product Details <ChevronRight className="inline ml-1" size={20} />
        </button>
      ...
      <section className="px-5 py-10 bg-gray-50">
        ...
        <div className="space-y-3 max-w-md mx-auto">
          {(content.problem_points || []).map((point, idx) => (
            <div key={`problem-${idx}`} className="flex items-start gap-3 bg-white p-4 rounded-xl shadow-sm">
      ...
      <section className="px-5 py-10">
        ...
        >
          See Full Product Details <ArrowRight className="inline ml-1" size={20} />
        </button>
      </section>
      ...
      <section className="px-5 py-10 bg-gradient-to-b from-purple-50 to-white">
        ...
        <div className="grid grid-cols-2 gap-3 max-w-md mx-auto">
          {[
            ...
            { name: 'Vitamin E', desc: 'Protects skin', color: 'from-green-500 to-emerald-500' },
          ].map((item, idx) => (
            <div key={`ingredient-${idx}`} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
      ...
      <section className="px-5 py-10">
        ...
        <div className="space-y-4 max-w-md mx-auto">
          {[
            ...
            <div key={`review-${idx}`} className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="ml-auto flex gap-0.5">
                  {[1,2,3,4,5].map(i => (
                    <Star key={`star-${idx}-${i}`} size={12} className="text-yellow-400 fill-yellow-400" />
                  ))}
                </div>
              </div>
      ...
      {/* Final CTA Section */}
      <section className="px-5 py-10 bg-gradient-to-r from-green-500 to-emerald-600 text-white">
        <div className="max-w-md mx-auto text-center">
          ...
          >
            View Product & Order - ₹{PREPAID_PRICE}
          </button>
