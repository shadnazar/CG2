    ...
    <div className="min-h-screen bg-white" data-testid="homepage">
      ...
      {kit && (
        <section className="relative py-12 sm:py-16 overflow-hidden" data-testid="complete-kit-section">
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-stone-50 via-emerald-50/40 to-amber-50/30" />
          <div className="absolute inset-0 opacity-50" style={{ backgroundImage: 'radial-gradient(circle at 15% 25%, rgba(16,185,129,0.12) 0%, transparent 45%), radial-gradient(circle at 85% 75%, rgba(250,204,21,0.10) 0%, transparent 45%)' }} />
            ...
            <div className="relative rounded-[28px] overflow-hidden bg-white ring-1 ring-emerald-100 shadow-2xl shadow-emerald-900/[0.08]">
              ...
              <div className="flex items-center justify-between px-5 sm:px-7 py-3 border-b border-emerald-50 bg-gradient-to-r from-emerald-50/60 via-amber-50/30 to-transparent">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] tracking-widest text-emerald-800 font-bold">BEST SELLER · LIMITED STOCK</span>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
                ...
                <div className="lg:col-span-5 p-6 sm:p-8 lg:p-10 bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-900 text-white relative">
                  ...
                  <div className="relative">
                    ...
                    <p className="text-sm text-emerald-100/80 mt-2 leading-relaxed">{kit.description}</p>
                    ...
                    <button
                      ...
                      className="group/btn mt-5 w-full relative overflow-hidden bg-amber-400 hover:bg-amber-300 text-emerald-950 font-black py-4 rounded-2xl text-sm tracking-wide shadow-2xl shadow-amber-900/20 transition-all hover:shadow-amber-400/30 hover:-translate-y-0.5 flex items-center justify-center gap-2"
      ...
      <div className="bg-gradient-to-r from-emerald-700 to-emerald-800 text-white py-2.5 px-4 text-center">
      ...
      <section className="max-w-7xl mx-auto px-4 py-8 sm:py-12" data-testid="products-section">
        <div className="text-center mb-6">
          ...
          <div className="w-12 h-0.5 bg-emerald-600 mx-auto mt-2.5 rounded-full" />
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
          {products.map(product => {
            ...
            return (
            <div key={product.slug} className="group bg-white rounded-2xl ring-1 ring-emerald-50 hover:ring-emerald-200 overflow-hidden hover:shadow-2xl hover:shadow-emerald-900/[0.08] hover:-translate-y-1 transition-all duration-300" data-testid={`product-card-${product.slug}`}>
              {product.badge && <div className={`text-[11px] font-bold px-3 py-1.5 text-center tracking-wide ${product.badge === 'Bestseller' ? 'bg-amber-400 text-amber-950' : product.badge === 'New Launch' ? 'bg-emerald-700 text-white' : 'bg-emerald-50 text-emerald-700'}`}>{product.badge.toUpperCase()}</div>}
              <Link to={`/product/${product.slug}`} className="block">
                <div className="aspect-square bg-gradient-to-br from-stone-50 to-emerald-50/40 flex items-center justify-center p-4 group-hover:scale-105 transition-transform duration-500">
                  {product.images?.[0] ? <img src={product.images[0]} alt="" className="w-full h-full object-contain" /> : <Sparkles className="w-10 h-10 text-emerald-200" />}
                </div>
              </Link>
              {/* Live activity strip */}
              ...
              <div className="p-3">
                <Link to={`/product/${product.slug}`}><h3 className="font-bold text-gray-900 text-sm leading-tight mb-1 group-hover:text-emerald-700 line-clamp-2 transition-colors">{product.short_name}</h3></Link>
                ...
                <div className="flex items-baseline gap-2 mb-1">
                  ...
                  <span className="text-xs font-bold text-emerald-700">{product.discount_percent}% Off</span>
                </div>
                {/* Coupon — brand amber */}
                ...
                <button onClick={(e) => { e.preventDefault(); handleAddToCart(product.slug); }} className="w-full bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold py-2.5 rounded-xl transition-all shadow-sm hover:shadow-md hover:shadow-emerald-900/10 flex items-center justify-center gap-1.5" data-testid={`add-to-cart-${product.slug}`}>
